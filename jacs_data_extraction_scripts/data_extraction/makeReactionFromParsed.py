...
class SmilesResolver:
    ...
    def getOldSolventClass(self, smiset):
        ...
        if len(klasses) == 3:
            if (
                "water" in klasses
                and "alcohols" in klasses
                and "aromatics" in klasses
            ):
    ...
    def detectBaseClass(self, smiset):
        ...
        if not detected:
            amines = [s for s in smiset if "N" in s and "Na" not in s]
            if amines:
                return "amine"
        if len(detected) > 1:
...
def parseArgs():
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="file with parsed reactions",
    )
    parser.add_argument(
        "--auxoutformat",
        choices=["none", "base7solv6", "base7solv13"],
        default="none",
    ...
    parser.add_argument(
        ...
        default=-1,
        type=float,
        help="for fake reactions set this yield",
    )
    parser.add_argument(
        "--outprefix",
        type=str,
        required=True,
        help="directory wihere results will be stored",
    )
    parser.add_argument(
        "--includewaste",
        action="store_true",
        help="if selected also smiles with unknown role will be included",
    )
    parser.add_argument(
        "--makecanonsmiles",
        action="store_true",
        help="make substrate and product canonical",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="create N folds if N < 1 all data in one id not CV",
    )
    parser.add_argument(
        "--conditionsasreactants",
        action="store_true",
        help="if selected conditions will be added to starting material sbs.cond>>prod instead of sbs>cond>prod",
    )
    parser.add_argument(
        "--outputformat",
        choices=["rx", "rxrecom", "GAT", "fp512r3"],
        default="rx",
        help="output format either full rx or csv for relGAT",
    )
...
def makeCatDict(rxlist, args):
    ...
    for rxline in rxlist:
        pdlig = frozenset(
            set(rxline["catalysts"]).union(set(rxline["ligands"]))
        )
        # pdlig = frozenset(set().union(set(rxline['ligands'])))
        if pdlig not in pdligDict:
...
def buildRxSmiles(dane, includeWaste):
    ...
    return (
        condlist,
        _makeSmiList(dane["bases"]),
        _makeSmiList(dane["solvents"]),
        dane["yield"],
    )
...
def parseLineToGAT(line, smiSolver, args, pdligName):
    ...
    if len(solv) >= 1:
        exactSolv = ".".join([str(sol) for sol in solv])
    else:
        exactSolv = "No Solvent"
    exactBase = ""

    exactBases = []
    for base in bs:
        if isinstance(base, str):
            exactBases.append(base)
        elif isinstance(base, (tuple, list, set)):
            exactBases.append(".".join([str(b) for b in base]))

    if len(exactBases) >= 1:
        exactBase = ",".join(exactBases)
    else:
        exactBase = "No Base"
    ...
    # ML = 'M0M'
    numML = 10
    B, S = smiSolver.getBaseSolvClassNum(
        baseClass, solvClass, args.auxoutformat
    )
    B = baseClass
    S = solvClass
    fakeT = "20"
...
def parseLineToFP(line, smiSolver, args, fplen=512, radius=3):
    ...
    fp1 = AllChem.GetMorganFingerprintAsBitVect(
        AllChem.MolFromSmiles(sbs[0]), radius, nBits=fplen
    ).ToBitString()
    _ = [vector.append(x) for x in fp1]
    fp2 = AllChem.GetMorganFingerprintAsBitVect(
        AllChem.MolFromSmiles(sbs[1]), radius, nBits=fplen
    ).ToBitString()
...
def parseLineToRx(line, smiSolver, args):
    ...
    if args.auxoutformat != "none":
        ...
        solvSameClass = basolv["solvents"][
            basolv["solventNames"].index(solvClass)
        ]
...
def printLines(lines, smiSolver, args):
    ...
    for idx, fold in enumerate(foldidx):
        ...
        if args.outputformat == "GAT":
            print(
                "Reactant1;Reactant2;Product;Yield;"
                + fakeT
                + "ML;B;S;exact_base;exact_solvent",
                file=fhmain,
            )
            print(
                "Reactant1;Reactant2;Product;Yield;"
        ...
        for lid, line in enumerate(lines):
            ...
            if args.outputformat == "GAT":
                print(
                    *parseLineToGAT(line, smiSolver, args, pdligDict),
                    sep=";",
                    file=outfile,
                )
                ...
                rxinfo = parseLineToFP(
                    line, smiSolver, args, fplen=512, radius=3
                )
