...
suzukiRx = AllChem.ReactionFromSmarts(
    "[c:3][BX3:4]([OX2H:5])[OX2H:6].[c:1][Br,I:2]>>[c:1][c:3].[*:2].[B:4]([O:5])[O:6]"
)
...
specialCasesConditions = {
    ("C1CCOC1", "C=C[CH2-]", "C=C[CH2-]", "Cl[Pd+]", "Cl[Pd+]", "O"): {
        "solvents": ("C1CCOC1", "O"),
        "Pd": [("C=C[CH2-]", "Cl[Pd+]")],
    },
    (
        ...
        "ligands": ("c1ccc(P(c2ccccc2)c2ccccc2)cc1",),
        "Pd": [
            ("O=C([O-])[O-]", "[Pd+2]"),
        ],
    },
    (
        "CC(C)(C)P(c1ccc[cH-]1)C(C)(C)C",
        "CC(C)(C)P(c1ccc[cH-]1)C(C)(C)C",
        "[Cl-]",
        "[Cl-]",
        "[Fe+2]",
    ...
    (
        ...
        "solvents": ["CS(C)=O", "CO", "C1COCCO1", "CCOC(C)=O"],
    },
    (
        "C1COCCO1",
        "O",
        ...
        "Pd": [
            ("[Cl-]", "[Cl-]", "[Cl-]", "[Cl-]", "[Na+]", "[Na+]", "[Pd+4]")
        ],
        "ligands": ("CC(C)(C)P(CCCS(=O)(=O)O)C(C)(C)C",),
    },
    (
        "C1CCOC1",
        "CC(C)c1cc(C(C)C)c(-c2cccc(P(C3CCCCC3)C3CCCCC3)c2)c(C(C)C)c1",
        "CC(C)c1cc(C(C)C)c(-c2ccccc2P(C2CCCCC2)C2CCCCC2)c(C(C)C)c1",
        "Cl[Pd+]",
        "Cl[Pd+]",
        "NCCc1[c-]cccc1",
        "Nc1ccccc1-c1[c-]cccc1",
    ): {
        "solvents": [
            "C1CCOC1",
        ],
        ...
        "Pd": [
            ("Cl[Pd+]", "NCCc1[c-]cccc1"),
            ("Cl[Pd+]", "Nc1ccccc1-c1[c-]cccc1"),
        ],
    },
    ("CC(=O)[O-]", "CC(=O)[O-]", "[Pd+2]", "c1ccc(-c2ccccc2)cc1"): {
        "ligands": [
...
class ReplaceCmd:
    ...
    def parseCondFile(fn):
        ...
        for line in fh:
            ...
            if len(elems) == 3 and elems[2]:
                canonsmi = elems[2]
            if "." in smi:
                retComplexDict[frozenset(smi.split("."))] = {
                    "raw": tuple(smi.split(".")),
                    "canon": tuple(canonsmi.split(".")),
                }
            else:
    ...
    def _isStoichCorrect(cmdPartsList, fullList):
        cmdPartsCount = {
            cmdPart: cmdPartsList.count(cmdPart)
            for cmdPart in set(cmdPartsList)
        }
        ...
        for cligand in self.ligandComplex:
            ...
            if DEBUG_LIGAND:
                print(
                    ...
                    smiList,
                    ":::",
                    cligand.issubset(smiSet),
                    self._isStoichCorrect(
                        self.ligandComplex[cligand]["raw"], smiList
                    ),
                )
            if cligand.issubset(smiSet) and self._isStoichCorrect(
                self.ligandComplex[cligand]["raw"], smiList
            ):
    ...
    def getIonicBase(self, smilesSet, debug=False):
        ...
        if smilesSet:
            for base in self.baseComplex:
                if debug:
                    print(
                        "BASE", base, base.issubset(smilesSet), "IN", smilesSet
                    )
    ...
    def getNormSmiles(self, smiles):
        ...
        for smi in smiles:
            ...
            if (
                smi in self.singleWrongToFull
                or noNumSmi in self.singleWrongToFull
            ):
                orygWrong.add(smi)
...
def parseRxFile(
    ...
    onlyWithYield=True,
    selectedRx=("suzuki",),
    columnToRead={"smiles": (0,), "yields": (1,)},
    ...
    for lid, line in enumerate(fh):
        ...
        if "notUsedSbs" in suzukiInfo or "notUsedProd" in suzukiInfo:
            print(
                "FFFFFF",
                rawSmiles,
                ".".join(suzukiInfo.get("notUsedSbs", [])),
                ".".join(suzukiInfo.get("notUsedProd", [])),
            )
        if "other" in suzukiInfo:
...
def makeCleanUp(suzukiInfo):
    if "other" in suzukiInfo and not suzukiInfo["bases"]:
        if len(suzukiInfo["other"]) == 1 and suzukiInfo["other"] in {
            "CC(=O)O"
        }:
            _ = suzukiInfo.pop("other")
            suzukiInfo["bases"].add(("CC(=O)[O-]", "[Na+]"))
        elif len(suzukiInfo["other"]) == 1 and suzukiInfo["other"] in {
            "O=P(O)(O)O"
        }:
            _ = suzukiInfo.pop("other")
            suzukiInfo["bases"].add(
                ("O=P([O-])([O-])[O-]", "[K+]", "[K+]", "[K+]")
            )
    return suzukiInfo
...
def addMissedConditionsFromSubstrate(
    replaceObj, infoDict, debug=False, removeAtomMapNum=True
):
    rmOther = []
...
def getSuzukiRoles(smiles, replaceObj):
    ...
    for i in range(
        5
    ):  # in some reaction ligands are repeated so removed it several time
...
def getSuzukiCompounds(sbsset, prodSet):
    candidateB = [
        s
        for s in sbsset
        if s.count("B") > s.count("Br") and s != "F[B-](F)(F)F"
    ]
    ...
    for bi, b in enumerate(molsB):
        for xi, x in enumerate(molsX):
            ...
            if trueProds:
                ...
                for p in trueProds:
                    usedProd.add(p)
    notUsedSbs = [s for s in sbsset if s not in usedSbs]
    notUsedProd = [s for s in prodSet if s not in usedProd]
...
def isSuzukiRx(b, x, expProds, prodSmis, verbose=False):
    try:
        ...
        mainSmiles = list(
            [s for s in set([Chem.MolToSmiles(p) for p in mainProd]) if s]
        )
        ...
        for main in mainProds:
            ...
            for poz, expProd in enumerate(expProds):
                if expProd.HasSubstructMatch(main) and main.HasSubstructMatch(
                    expProd
                ):
...
def getPdCat(catSmi):
    ...
    nopdsmi = [s for s in catSmi if "Pd" not in s]
    if DEBUG_PD:
...
def saveToFiles(rxlist, prefix, singleProduct=True, debug=False):
    ...
    for rx in rxlist:
        ...
        if (
            sbs1.HasSubstructMatch(het1)
            or sbs2.HasSubstructMatch(het1)
            or sbs1.HasSubstructMatch(het2)
            or sbs2.HasSubstructMatch(het2)
            or sbs1.HasSubstructMatch(het3)
            or sbs2.HasSubstructMatch(het3)
            or sbs1.HasSubstructMatch(het4)
            or sbs2.HasSubstructMatch(het4)
        ):
            fraw = frawhet
            fclear = fclearhet
            fparsed = fparsedhet
        ...
        if debug == True:
            if "other" in rx["parsed"]:
                trueOther = [
                    x for x in rx["parsed"]["other"] if x not in otherIgnore
                ]
                showIt = [
                    ["[NH4+]", "[Cl-]"],
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract data from USPTO dataset"
    )
    parser.add_argument(
        "-i",
        "--input_csvs",
